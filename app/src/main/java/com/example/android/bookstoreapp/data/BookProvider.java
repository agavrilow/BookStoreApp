...
public class BookProvider extends ContentProvider {
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        ...
        Integer quantity = values.getAsInteger(BookEntry.COLUMN_BOOK_QTY);
    ...
    private int updateBook(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(BookEntry.COLUMN_SUPPLIER_NAME)) {
            String supName = values.getAsString(BookEntry.COLUMN_SUPPLIER_NAME);
            if (supName == null) {
        ...
        if (values.containsKey(BookEntry.COLUMN_SUPPLIER_PHONE)) {
            String supPhone = values.getAsString(BookEntry.COLUMN_SUPPLIER_PHONE);
            if (supPhone == null) {
        ...
        if (values.containsKey(BookEntry.COLUMN_BOOK_QTY)) {
            Integer quantity = values.getAsInteger(BookEntry.COLUMN_BOOK_QTY);
            if (quantity != null && quantity < 0) {
                throw new IllegalArgumentException("Book requires valid quantity");
            }
